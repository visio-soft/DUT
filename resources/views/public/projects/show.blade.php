            ...
            <div class="flex items-center space-x-4">
                ...
                <div>
                    ...
                    <p class="text-gray-600 mt-1">
                        ...
                        <span class="text-blue-600">{{ $project->category->name ?? __('common.uncategorized') }}</span>
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                    ...
                    <!-- Project Description -->
                    <div class="mb-6">
                        <h3 class="text-xl font-semibold text-gray-900 mb-3">{{ __('common.project_description') }}</h3>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-gray-50 rounded-lg">
                        @if($project->estimated_duration)
                            <div class="flex items-center space-x-2">
                                ...
                                <span class="font-medium">{{ $project->estimated_duration }} {{ __('common.days') }}</span>
                            </div>
                        @endif
                ...
                @if($project->suggestions->count() > 0)
                    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                        <h3 class="text-xl font-semibold text-gray-900 mb-4 flex items-center">
                            ...
                            {{ __('common.linked_suggestions') }} ({{ $project->suggestions->count() }})
                        </h3>
                                            ...
                                            <div class="flex items-center space-x-4 text-xs text-gray-500">
                                                ...
                                                @if($suggestion->likes_count > 0)
                                                    <span class="flex items-center">
                                                        ...
                                                        {{ $suggestion->likes_count }} {{ __('common.likes') }}
                                                    </span>
                                                @endif
                                            </div>
                                        </div>
            ...
            <div class="lg:col-span-1">
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 sticky top-4">
                    <!-- Project Info -->
                    <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ __('common.project_information') }}</h3>

                    <div class="space-y-4">
                        <!-- Category -->
                        <div>
                            <span class="text-sm font-medium text-gray-500">{{ __('common.category') }}</span>
                            <p class="text-sm text-gray-900 mt-1">{{ $project->category->name ?? __('common.uncategorized') }}</p>
                        </div>

                        <!-- Creator -->
                        @if($project->createdBy)
                        <div>
                            <span class="text-sm font-medium text-gray-500">{{ __('common.creator') }}</span>
                        ...
                        @if($project->start_date || $project->end_date)
                            <!-- Project Timeline -->
                            <div>
                                <span class="text-sm font-medium text-gray-500">{{ __('common.project_duration') }}</span>
                                <div class="text-sm text-gray-900 mt-1">
                                    @if($project->start_date)
                                        <p>{{ __('common.start') }}: {{ \Carbon\Carbon::parse($project->start_date)->format('d.m.Y') }}</p>
                                    @endif
                                    @if($project->end_date)
                        ...
                        <div class="pt-4 border-t border-gray-200">
                            ...
                            <div class="text-sm text-gray-900 mt-2 space-y-1">
                                <p>{{ $project->likes_count }} {{ __('common.likes') }}</p>
                                <p>{{ $project->suggestions->count() }} {{ __('common.linked_suggestions') }}</p>
                                <p>{{ __('common.created_time_ago', ['time' => $project->created_at->diffForHumans()]) }}</p>
                            </div>
                        </div>
