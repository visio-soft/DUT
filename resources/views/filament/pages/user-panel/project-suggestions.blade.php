...
@section('content')
    ...
    <style>
        ...
        .sticky-project-header.is-stuck .sticky-divider {
            ...
            /* Reduced from 15px for subtler movement */
        ...
        .sticky-project-header .sticky-header-content {
            ...
            align-items: center;
            justify-content: center;
            /* Default center for mobile/smaller screens */
            gap: 0.75rem;
            max-width: 1400px;
            margin: 0 auto;
            padding: 0 1.5rem;
        ...
        @media (min-width: 1024px) {
            ...
            .sticky-project-header.is-stuck .sticky-header-content {
                padding-left: 320px;
                /* Offset for sidebar (approx 300px + gap) */
            }
        }
        ...
        <div class="sticky-header-content">
            ...
            @if($project->surveys->where('status', true)->isNotEmpty())
                ...
                <a href="#" onclick="Livewire.dispatch('openSurveyModal', { surveyId: {{ $activeSurvey->id }} }); return false;"
                    class="sticky-info-box {{ $userHasAnsweredSurvey ? 'gray' : 'green' }}" style="text-decoration: none;">
                    <svg class="sticky-info-icon" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <div style="display: flex; flex-direction: column; gap: 0.125rem; text-align: left;">
                        @foreach($stickyActiveFilters as $key => $value)
                            <div style="font-size: 0.75rem; font-weight: 500; line-height: 1.2;">
                                <span
                                    style="opacity: 0.8;">{{ $stickyFilterLabelMap[$key] ?? ucfirst(str_replace('_', ' ', $key)) }}:</span>
                ...
                <div class="filters-sidebar-wrapper">
                    <div id="user-filter-panel" class="filters-card {{ $startCollapsed ? 'collapsed' : '' }}">
                        ...
                        <div class="filters-content">
                            <form id="suggestions-filters-form" method="GET"
                                action="{{ route('user.project.suggestions', $project->id) }}">
                    ...
                    <div class="tree-view">
                        ...
                        <div style="space-y: 0.5rem;">
                            @forelse($project->suggestions->sortByDesc(function ($s) {
                                return $s->likes->count(); }) as $sidebarSuggestion)
                                <div class="tree-suggestion" onclick="scrollToSuggestion({{ $sidebarSuggestion->id }})"
                                    style="border-bottom: 1px solid var(--green-100); padding: 0.5rem; cursor: pointer;">
                                    <svg style="width: 0.875rem; height: 0.875rem; margin-right: 0.5rem; color: var(--green-500);"
                                        fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                            d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m2.25 0H5.25A2.25 2.25 0 0 0 3 5.25v13.5A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V8.25A2.25 2.25 0 0 0 18.75 6H16.5a2.25 2.25 0 0 1-2.25-2.25V3.75a2.25 2.25 0 0 0-2.25-2.25Z" />
                                    </svg>
                                    <span
                                        style="font-size: 0.8125rem; font-weight: 500; color: var(--gray-900); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; flex: 1;">{{ Str::limit($sidebarSuggestion->translated_title, 22) }}</span>
                                    <span
                                        style="margin-left: auto; display: flex; align-items: center; font-size: 0.75rem; color: var(--gray-500);">
                            ...
                            @empty
                                <p style="font-size: 0.9rem; color: var(--gray-500); text-align: center;">
                                    {{ __('common.no_suggestions_for_project') }}</p>
                            @endforelse
                        </div>
                    </div>
                </div>
                    ...
                    @if($project->suggestions->count() > 0)
                        <div class="suggestions-container">
                            <div class="d-flex" style="flex-direction: column; gap: 2rem;">
                                @foreach($project->suggestions->sortByDesc(function ($suggestion) {
                                    return $suggestion->likes->count(); }) as $index => $suggestion)
                                    @php
                                    ...
                                    <div id="suggestion-{{ $suggestion->id }}"
                                        ...
                                        @if($isProjectCompleted)
                                            @if($isWinner)
                                                <div
                                                    style="position: absolute; top: 0; left: 0; right: 0; background: linear-gradient(135deg, #22c55e 0%, #16a34a 100%); color: white; padding: 0.5rem 1rem; z-index: 10; display: flex; align-items: center; justify-content: center; gap: 0.5rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; font-size: 0.875rem; box-shadow: 0 2px 8px rgba(34, 197, 94, 0.4);">
                                            ...
                                            @else
                                                <div
                                                    style="position: absolute; top: 0; left: 0; right: 0; background: linear-gradient(135deg, #6b7280 0%, #4b5563 100%); color: white; padding: 0.5rem 1rem; z-index: 10; display: flex; align-items: center; justify-content: center; gap: 0.5rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em; font-size: 0.75rem;">
                                        ...
                                        <div style="position: relative; z-index: 3; padding: 2rem; color: white;">
                                            <div
                                                ...
                                                <div style="display: flex; align-items: center;">
                                                    <svg style="width: 1.5rem; height: 1.5rem; margin-right: 0.75rem; color: rgba(255,255,255,0.9);"
                                                        fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                                            d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m2.25 0H5.25A2.25 2.25 0 0 0 3 5.25v13.5A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 18 18.75V10.5m-10.5 6L21 3m0 0h-5.25M21 3v5.25" />
                                                    </svg>
                                                    <h2
                                                        style="font-size: 1.75rem; font-weight: 700; color: white; margin: 0; text-shadow: 0 2px 4px rgba(0,0,0,0.5);">
                                                        {{ $suggestion->translated_title }}</h2>
                                                </div>
                                            </div>
                                            ...
                                            @if($suggestion->description)
                                                <p
                                                    style="font-size: 1rem; color: rgba(255,255,255,0.9); margin-bottom: 1.5rem; text-shadow: 0 1px 2px rgba(0,0,0,0.5); line-height: 1.5;">
                                                    ...
                                                    {{ $suggestion->translated_description }}
                                                </p>
                                            @endif
                                            ...
                                            <div style="display: flex; gap: 1rem; align-items: center;">
                                                ...
                                                <button
                                                    ...
                                                    data-suggestion-id="{{ $suggestion->id }}" data-project-id="{{ $project->id }}"
                        ...
                        <div class="text-center section-padding-lg">
                            <div class="user-card" style="max-width: 400px; margin: 0 auto; padding: 3rem;">
                                ...
                                <div style="display: flex; align-items: center; justify-content: center; margin-bottom: 1rem;">
                                    ...
                                    <h3 style="font-size: 1.125rem; font-weight: 600; color: var(--gray-900); margin: 0;">
                                        {{ __('common.no_suggestions_for_project') }}</h3>
                                </div>
