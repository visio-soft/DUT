...
@section('title', $suggestion->translated_title . ' - DUT Vote')

@section('content')
    <!-- Header Section with Background -->
    <section class="section-padding bg-transparent"
        style="padding: 1rem 0 0.5rem 0; position: relative; overflow: hidden; min-height: 12rem;">
        @if($hasBackgroundImages ?? false)
            @if($randomBackgroundImage)
                <!-- Single Random Background Image -->
                <div class="background-image-container">
                    <img src="{{ $randomBackgroundImage }}" alt="{{ __('common.suggestion_details') }}"
                        class="background-image-main" loading="eager">
                </div>
            @endif
        ...
        <div class="w-full relative z-10">
            <div class="user-container py-1">
                <nav
                    class="flex items-center gap-2 text-sm font-medium overflow-x-auto whitespace-nowrap scrollbar-hide pb-1">
                    <!-- Home -->
                    <a href="{{ route('user.index') }}"
                        class="flex items-center px-3 py-1 rounded-full bg-[#f0fdf4] text-green-800 hover:bg-green-100 transition-colors group shadow-sm border border-green-50">
                        <svg class="w-4 h-4 mr-1.5 text-green-600 group-hover:text-green-700 transition-colors" fill="none"
                    ...
                    <a href="{{ route('user.projects') }}"
                        class="px-3 py-1 rounded-full bg-[#f0fdf4] text-green-800 hover:bg-green-100 transition-colors shadow-sm border border-green-50">
                        {{ __('common.projects') }}
                    </a>
                    ...
                    <!-- Current Page -->
                    <span class="px-3 py-1 rounded-full bg-green-600 text-white font-medium shadow-sm truncate max-w-xs"
                        title="{{ $suggestion->translated_title }}">
                    ...
                    <div class="content-spacing-lg">
                        <div style="flex: 1;">
                            <div
                                ...
                                <h1 class="detail-title" style="margin-bottom: 0;">{{ $suggestion->translated_title }}</h1>
                                @if($suggestion->status)
                                    <span
                                        style="padding: 0.5rem 1rem; border-radius: 9999px; font-size: 0.875rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.025em;
                            ...
                            <div style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: 1rem;">
                                <span
                                    style="font-size: 0.875rem; color: var(--gray-500);">{{ __('common.project') }}:</span>
                                <span
                                    style="display: inline-flex; align-items: center; padding: 0.25rem 0.75rem; border-radius: var(--radius-xl); font-size: 0.875rem; background: var(--green-100); color: var(--green-800);">
                            ...
                            <div class="detail-meta">
                                ...
                                @if($suggestion->category && $suggestion->category->end_datetime)
                                    ...
                                    <div style="display: flex; align-items: center;">
                                        ...
                                        <span
                                            style="color: {{ $isExpired ? 'var(--red-600)' : 'var(--green-600)' }}; font-weight: 600;">
                                            @if($isExpired)
                    ...
                    @if($suggestion->address || $suggestion->city || $suggestion->country)
                        <div class="detail-location">
                            <div style="display: flex; align-items: start; gap: 0.5rem;">
                                <svg style="width: 1.25rem; height: 1.25rem; color: var(--gray-400); margin-top: 0.125rem;"
                                ...
                                <div>
                                    <h4 style="font-weight: 500; color: var(--gray-900); margin-bottom: 0.25rem;">
                                        {{ __('common.location') }}
                                    </h4>
                ...
                <div class="d-grid" style="grid-template-columns: 2fr 1fr; gap: 2rem;">
                    ...
                    <div class="detail-content">
                            ...
                            <div class="content-spacing" style="position: relative;">
                                <div style="position: relative; cursor: pointer; overflow: hidden; border-radius: var(--radius-lg);"
                                    ...
                                    <img id="suggestion-image" src="{{ $suggestion->getFirstMediaUrl('images') }}"
                                        alt="{{ $suggestion->translated_title }}" class="suggestion-preview-image"
                                        onclick="openImageModal()"
                                    ...
                                    <!-- Zoom Overlay -->
                                    <div id="zoom-overlay"
                                        style="position: absolute; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.4); display: flex; align-items: center; justify-content: center; opacity: 0; transition: opacity 0.3s ease; border-radius: var(--radius-lg); pointer-events: none;">
                                        <div
                                            style="background: rgba(255,255,255,0.9); padding: 0.75rem; border-radius: 50%; backdrop-filter: blur(8px);">
                                            <svg style="width: 2rem; height: 2rem; color: var(--gray-700);" fill="none"
                        ...
                        <div class="content-spacing">
                            <h3
                                style="font-size: 1.25rem; font-weight: 600; color: var(--gray-900); margin-bottom: 1rem; display: flex; align-items: center; gap: 0.5rem;">
                                <svg style="width: 1.25rem; height: 1.25rem; color: var(--blue-600);" fill="none"
                                    stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                            ...
                            <div style="color: var(--gray-600); font-size: 1rem; line-height: 1.7;">
                                {!! nl2br(e($suggestion->translated_description)) !!}
                            </div>
                        </div>
                        ...
                        <div style="border-top: 1px solid var(--green-100); padding-top: 2rem;">
                            <h3
                                style="font-size: 1.25rem; font-weight: 600; color: var(--gray-900); margin-bottom: 1.5rem; display: flex; align-items: center; gap: 0.5rem;">
                                <svg style="width: 1.25rem; height: 1.25rem; color: var(--blue-600);" fill="none"
                                    stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                                ...
                                {{ __('common.comments') }}
                                ({{ $suggestion->approvedComments->count() }}{{ $userPendingComments->count() > 0 ? ' + ' . $userPendingComments->count() . ' ' . __('common.pending') : '' }})
                            </h3>
                                    ...
                                    @if($userPendingComments->count() > 0)
                                        @foreach($userPendingComments as $comment)
                                            <div class="comment-card"
                                                style="background: var(--yellow-50); border: 1px solid var(--yellow-200); position: relative;">
                                                <!-- Pending Badge -->
                                                <div style="position: absolute; top: 0.75rem; right: 0.75rem;">
                                                    <span
                                                        style="display: inline-flex; align-items: center; gap: 0.25rem; padding: 0.25rem 0.75rem; background: var(--yellow-100); color: var(--yellow-800); font-size: 0.75rem; font-weight: 500; border-radius: var(--radius-xl); border: 1px solid var(--yellow-300);">
                                                ...
                                                @if($comment->parent_id)
                                                    <div
                                                        style="margin-bottom: 0.75rem; padding: 0.5rem 0.75rem; background: var(--blue-50); border: 1px solid var(--blue-200); border-radius: var(--radius-md); display: flex; align-items: center; gap: 0.5rem;">
                                                        <svg style="width: 1rem; height: 1rem; color: var(--blue-600);" fill="none"
                                                            stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                                d="M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V16.5M16.5 12 12 16.5m0 0L7.5 12m4.5 4.5V3" />
                                                        </svg>
                                                        <span style="font-size: 0.85rem; color: var(--blue-700);">
                                                            <strong>{{ $comment->parent->user->name ?? __('common.anonymous') }}</strong>
                                                            {{ __('common.reply_to_user') }}
                                                        </span>
                                                    </div>
                                                @endif
                                                ...
                                                <div class="comment-header">
                                                    ...
                                                    <div>
                                                        <h4 class="comment-author" style="color: var(--yellow-800);">
                                                            {{ $comment->user->name ?? 'Anonim' }} <span
                                                                style="font-weight: 400; font-size: 0.9em;">({{ __('common.you') }})</span>
                                                        </h4>
                                                        <span class="comment-date"
                                                            style="color: var(--yellow-600);">{{ $comment->created_at->diffForHumans() }}</span>
                                                    </div>
                                                </div>
                                                <p class="comment-content" style="color: var(--yellow-800);">
                                                    {{ $comment->translated_comment }}</p>

                                                <div
                                                    style="margin-top: 1rem; padding-top: 0.75rem; border-top: 1px solid var(--yellow-200); display: flex; align-items: center; gap: 0.5rem;">
                                                    <svg style="width: 1rem; height: 1rem; color: var(--yellow-600);" fill="none"
                                                        stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                                            d="m11.25 11.25.041-.02a.75.75 0 0 1 1.063.852l-.708 2.836a.75.75 0 0 0 1.063.853l.041-.021M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Zm-9-3.75h.008v.008H12V8.25Z" />
                                                    </svg>
                                        ...
                                        <div class="comment-card">
                                            <div class="comment-header">
                                                ...
                                                <div style="flex: 1;">
                                                    <h4 class="comment-author">
                                                        {{ $comment->user->name ?? 'Anonim' }}@if(Auth::check() && $comment->user_id === Auth::id())
                                                            <span
                                                        style="font-weight: 400; font-size: 0.9em;">({{ __('common.you') }})</span>@endif
                                                    </h4>
                                                    <span class="comment-date">{{ $comment->created_at->diffForHumans() }}</span>
                                                </div>
                                            </div>
                                            <p class="comment-content">{{ $comment->translated_comment }}</p>

                                            <!-- Comment Actions -->
                                            <div class="comment-actions"
                                                style="margin-top: 0.75rem; padding-top: 0.75rem; border-top: 1px solid var(--gray-100); display: flex; align-items: center; justify-content: space-between;">
                                                <div style="display: flex; align-items: center; gap: 1rem;">
                                                    @auth
                                                        <button onclick="toggleCommentLike({{ $comment->id }})"
                                                            ...
                                                            <svg style="width: 0.875rem; height: 0.875rem; {{ $comment->likes->where('user_id', Auth::id())->count() > 0 ? 'fill: currentColor;' : 'fill: none;' }}"
                                                                stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                                                                <path stroke-linecap="round" stroke-linejoin="round"
                                                                    d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12Z" />
                                                            </svg>
                                                    ...
                                                    @else
                                                        <span style="font-size: 0.875rem; color: var(--gray-500);">
                                                            {{ __('common.login_for_like_reply') }} <a href="{{ route('user.login') }}"
                                                                style="color: var(--blue-600);">{{ __('common.log_in') }}</a>
                                                        </span>
                                                    @endauth
                                                </div>
                                            ...
                                            @auth
                                                <div id="reply-form-{{ $comment->id }}" class="reply-form"
                                                    style="display: none; margin-top: 1rem; padding: 1rem; background: var(--gray-50); border-radius: var(--radius-md); border: 1px solid var(--gray-200);">
                                                    <form onsubmit="submitReply(event, {{ $comment->id }})">
                                                        @csrf
                                                        <div style="margin-bottom: 0.75rem;">
                                                            <textarea name="comment" id="reply-text-{{ $comment->id }}"
                                            ...
                                            @if($comment->approvedReplies->count() > 0)
                                                <div class="replies-section"
                                                    style="margin-top: 1.5rem; margin-left: 2.5rem; padding-left: 1rem; border-left: 3px solid var(--green-200);">
                                                    @foreach($comment->approvedReplies as $reply)
                                                        <div class="reply-card"
                                                            style="margin-bottom: 1rem; padding: 1rem; background: var(--green-50); border-radius: var(--radius-md); border: 1px solid var(--green-200);">
                                                            <div class="comment-header" style="margin-bottom: 0.5rem;">
                                                                <div class="comment-avatar"
                                                                    style="width: 2rem; height: 2rem; background: var(--green-300); color: var(--green-800); font-size: 0.75rem;">
                                                                    {{ substr($reply->user->name ?? 'A', 0, 1) }}
                                                                </div>
                                                                <div style="flex: 1;">
                                                                    <h5
                                                                        style="font-size: 0.9rem; font-weight: 600; color: var(--green-800); margin: 0;">
                                                                        {{ $reply->user->name ?? 'Anonim' }}@if(Auth::check() && $reply->user_id === Auth::id())
                                                                            <span
                                                                        style="font-weight: 400; font-size: 0.85em;">({{ __('common.you') }})</span>@endif
                                                                    </h5>
                                                                    <span
                                                                        style="font-size: 0.8rem; color: var(--green-600);">{{ $reply->created_at->diffForHumans() }}</span>
                                                                </div>
                                                            </div>
                                                            <p
                                                                style="margin: 0 0 0.75rem 0; font-size: 0.9rem; color: var(--green-800); line-height: 1.5;">
                                                                {{ $reply->comment }}
                                                            </p>

                                                            <!-- Reply Actions -->
                                                            <div
                                                                style="display: flex; align-items: center; justify-content: space-between; padding-top: 0.5rem; border-top: 1px solid var(--green-200);">
                                                                <div>
                                                                    @auth
                                                                        <button onclick="toggleCommentLike({{ $reply->id }})"
                                                                            ...
                                                                            onmouseover="this.style.transform='scale(1.05)'"
                                                                            onmouseout="this.style.transform='scale(1)'">
                                                                            <svg style="width: 0.75rem; height: 0.75rem; {{ $reply->likes->where('user_id', Auth::id())->count() > 0 ? 'fill: currentColor;' : 'fill: none;' }}"
                                                                                stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                                                                                <path stroke-linecap="round" stroke-linejoin="round"
                                                                                    d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12Z" />
                                                                            </svg>
                            ...
                            @auth
                                ...
                                <div id="comment-form"
                                    style="margin-top: 2rem; padding: 1.5rem; background: var(--gray-50); border-radius: var(--radius-lg); border: 1px solid var(--gray-200);">
                                    <h4
                                        style="font-size: 1rem; font-weight: 600; color: var(--gray-900); margin-bottom: 1rem; display: flex; align-items: center; gap: 0.5rem;">
                                        <svg style="width: 1.25rem; height: 1.25rem; color: var(--blue-600);" fill="none"
                                            stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                                    ...
                                    <form id="comment-submit-form" onsubmit="submitComment(event)">
                                        ...
                                        <div style="margin-bottom: 1rem;">
                                            <textarea name="comment" id="comment-text"
                                                placeholder="{{ __('common.comment_placeholder') }}" required maxlength="1000"
                                        ...
                                        <div style="display: flex; gap: 0.75rem;">
                                            ...
                                            <button type="reset" onclick="resetCommentForm()" class="btn btn-secondary"
                                                style="display: inline-flex; align-items: center; gap: 0.5rem;">
                                                <svg style="width: 1rem; height: 1rem;" fill="none" stroke="currentColor"
                                                    stroke-width="1.5" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <div class="detail-sidebar">
                        ...
                        <div class="content-spacing">
                            <h4
                                style="font-weight: 600; color: var(--gray-900); margin-bottom: 1rem; display: flex; align-items: center; gap: 0.5rem;">
                                <svg style="width: 1.25rem; height: 1.25rem; color: var(--blue-600);" fill="none"
                                    stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                            ...
                            <div class="stats-grid">
                                <div class="stat-item">
                                    <span class="stat-label" style="display: flex; align-items: center; gap: 0.5rem;">
                                        <svg style="width: 1rem; height: 1rem; color: var(--red-500);" fill="none"
                                            stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12Z" />
                                        </svg>
                                ...
                                <div class="stat-item">
                                    <span class="stat-label" style="display: flex; align-items: center; gap: 0.5rem;">
                                        <svg style="width: 1rem; height: 1rem; color: var(--blue-500);" fill="none"
                                            stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                d="M8.625 12a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H8.25m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H12m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0h-.375M21 12c0 4.556-4.03 8.25-9 8.25a9.764 9.764 0 0 1-2.555-.337A5.972 5.972 0 0 1 5.41 20.97a5.969 5.969 0 0 1-.474-.065 4.48 4.48 0 0 0 .978-2.025c.09-.457-.133-.901-.467-1.226C3.93 16.178 3 14.189 3 12c0-4.556 4.03-8.25 9-8.25s9 3.694 9 8.25Z" />
                                        </svg>
                                ...
                                <div class="stat-item">
                                    <span class="stat-label" style="display: flex; align-items: center; gap: 0.5rem;">
                                        <svg style="width: 1rem; height: 1rem; color: var(--gray-500);" fill="none"
                                            stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 0 1 2.25-2.25h13.5A2.25 2.25 0 0 1 21 7.5v11.25m-18 0A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75m-18 0v-7.5A2.25 2.25 0 0 1 5.25 9h13.5A2.25 2.25 0 0 1 21 11.25v7.5" />
                                        </svg>
                        ...
                        @if($suggestion->likes->count() > 0)
                            <div class="content-spacing">
                                <h4
                                    style="font-weight: 600; color: var(--gray-900); margin-bottom: 1rem; display: flex; align-items: center; gap: 0.5rem;">
                                    <svg style="width: 1.25rem; height: 1.25rem; color: var(--purple-600);" fill="none"
                                        stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                ...
                                <div class="d-flex" style="flex-direction: column; gap: 0.5rem;">
                                    @foreach($suggestion->likes->take(5) as $like)
                                        <div style="display: flex; align-items: center; gap: 0.5rem; font-size: 0.875rem;">
                                            <div
                                                style="width: 1.5rem; height: 1.5rem; background: var(--green-300); border-radius: 50%; display: flex; align-items: center; justify-content: center;">
                        ...
                        <div class="d-flex" style="flex-direction: column; gap: 0.75rem;">
                            ...
                            <a href="{{ route('user.projects') }}" class="btn btn-primary" style="justify-content: center;">
                                <svg style="width: 1rem; height: 1rem; margin-right: 0.5rem;" fill="none"
                                    stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round"
    ...
    <script>
        ...
        <div id="image-modal"
            style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.95); z-index: 9999; justify-content: center; align-items: center; opacity: 0; transition: opacity 0.3s ease; padding: 1rem;"
            ...
            <div class="modal-content"
                ...
                <button onclick="closeImageModal()"
                    style="position: absolute; top: 1rem; right: 1rem; background: rgba(255,255,255,0.15); border: none; color: white; width: 3rem; height: 3rem; border-radius: 50%; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 1.5rem; backdrop-filter: blur(10px); transition: all 0.3s ease; z-index: 10;"
                    ...
                    <svg style="width: 1.75rem; height: 1.75rem;" fill="none" stroke="currentColor" stroke-width="2.5"
                        viewBox="0 0 24 24">
                ...
                <div id="zoom-instructions"
                    style="position: absolute; top: 1rem; left: 1rem; background: rgba(0,0,0,0.7); color: white; padding: 0.75rem 1rem; border-radius: var(--radius-lg); font-size: 0.875rem; backdrop-filter: blur(10px); z-index: 10; opacity: 1; transition: opacity 0.3s ease;">
                    <div style="display: flex; align-items: center; gap: 0.5rem;">
                ...
                <div id="image-container"
                    style="width: 100%; height: 85%; display: flex; align-items: center; justify-content: center; overflow: hidden; cursor: zoom-in; position: relative;">
                    <img id="modal-image" src="" alt="{{ $suggestion->translated_title ?? __('common.suggestion_image') }}"
                        style="max-width: 95%; max-height: 95%; width: auto; height: auto; object-fit: contain; border-radius: var(--radius-xl); box-shadow: 0 25px 80px rgba(0,0,0,0.7); transform: scale(0.95); transition: transform 0.4s ease; cursor: zoom-in;">
                </div>

                <!-- Image Title -->
                <div
                    style="background: rgba(55, 65, 81, 0.85); padding: 1.25rem 2.5rem; border-radius: var(--radius-xl); margin-top: 1.5rem; backdrop-filter: blur(12px); text-align: center; box-shadow: 0 15px 50px rgba(0,0,0,0.4); max-width: 90%; width: fit-content; border: 1px solid rgba(255,255,255,0.1);">
                    <h3
                        style="color: white; font-size: 1.5rem; font-weight: 700; margin: 0; line-height: 1.2; display: flex; align-items: center; justify-content: center; gap: 0.75rem;">
                        <svg style="width: 1.5rem; height: 1.5rem; color: #60a5fa;" fill="none" stroke="currentColor"
                            stroke-width="1.5" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round"
                        ...
                        {{ $suggestion->translated_title ?? __('common.suggestion_image') }}
                    </h3>
                    @if($suggestion->category)
                        <p
                            style="color: rgba(255,255,255,0.8); font-size: 1rem; margin: 0.75rem 0 0 0; font-weight: 500; display: flex; align-items: center; justify-content: center; gap: 0.5rem;">
